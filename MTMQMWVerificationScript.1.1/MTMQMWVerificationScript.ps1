...
function QMWCheckSharePointFeatures([string]$WAOrRC, [string]$QMWSiteURL)
{
#QMWCheckSharePointFeatures: Check SharePoint Features - Michael
	...
	if (($SharePointFeaturesArray -ne $null) -or ($SharePointFeaturesArray.Count -gt 0))
	{
		...
		foreach ($featureToCheck in $SharePointFeaturesArray)
		{
			...
			#Write-Host $featureToCheckScope
			if($SiteCollection -eq $WAOrRC)
			{
				if ($SubSiteURLCode -ne "_NOT_APPLICABLE_")
				{
					$targetURL = $QMWSiteURL + $SubSiteURLCode
				}
				else
				{
					$targetURL = $QMWSiteURL
				}
...
function QMWDataConnection ([string]$WAOrRC, [string]$QMWSiteURL)
{
...
{
#QMWCheckInfoPathFormServices: Check InfoPath Form Services in Central Admin  - Michael
...
function QMWAttachmentSeperator ([string]$WAOrRC, [string]$QMWSiteURL)
{
	Write-Output "`n--- Checking Attachment Seperator ---"
	$items =  $XMLConfig.XML.AttachmentSeperator.File
	...
	$cmmSiteURL = $targetURL + "CMM"
	$cmmWeb = Get-SPWeb ($cmmSiteURL)
	
	foreach($item in $items){
		$CMMSiteAssets = $cmmWeb.Lists[$item.ListName]
		$row = $AttachmentSeperatorTable.NewRow()
		$row["File Name"] = $item.FileName
		$row["List Name"] = $item.ListName
		$row["Expected Value"] = "File Exists"
		$row["Actual Value"] = "File does not exist"
		$row["Pass or Fail"] = "FAIL"
		$row["Date"] = $Date
		
		$filesInList = $CMMSiteAssets.Items.Name
		if ($filesInList -contains $item.FileName)
		{
			$row["Actual Value"] = "File Exists"
			$row["Pass or Fail"] = "PASS"
		}
		$AttachmentSeperatorTable.Rows.Add($row)
	}
	#$AttachmentSeperatorTable  | format-table -AutoSize
}

function QMWCheckInfoPathForms
{
#QMWCheckInfoPathForms: Check Form Templates library in QMW to see what forms have been deployed  - Michael
	...
	if (($InfoPathFormsArray -ne $null) -or ($InfoPathFormsArray.Count -gt 0))
	{
		...
		$SPSiteQMW = Get-SPSite $TargetSiteCollectionURL
		$QMWrootweb = $SPSiteQMW.rootweb
		$InfoPathFormTemplateLibrary = $QMWrootweb.Lists["Form Templates"]
...
{
#QMWCheckSiteSettings: Check QMW Site Settings - Simon
# Parameters:
		...
		foreach ($SettingToCheck in $SiteSettingsArray)
		{
			...
			if ($SiteCollection -eq $WAOrRC)
			{
				...
				elseif($SettingToCheckSettingType -eq "Site Collection Audit Settings")
				{
					...
					if($SPSiteQMW.Audit.AuditFlags -eq $SettingToCheckRequiredValue)
					{
				...
				elseif($SettingToCheckSettingType -eq "Content Organizer Rule")
				{
					...
					{
						$List = $QMWrootweb.lists["Content Organizer Rules"]
...
{
#QMWCheckLibrarySettings: Check QMW Library Settings  - Simon
# Parameters:
# 	-> WAOrRC: Whether it is the WA (Work Area) or RC (Records Center) that is to be verified
# 	-> QMWSiteURL: URL of the QMW site collection you want to verify
...
function QMWCheckWorkflowConstants([string]$WAOrRC, [string]$QMWSiteURL)
{
#QMWCheckWorkflowConstants: Check Workflow Constants - Shovan
	...
	if (($workflowConstantSettings -ne $null) -or ($workflowConstantSettings.Count -gt 0))
	{
		...
		foreach ($settingToCheck in $workflowConstantSettings)
		{
			...
			if ($settingToCheckConstantType -eq $workflowConstantType)
			{
				...
				{
					if (($settingToCheckName -match "CDM") -or ($settingToCheckName -match "CMM"))
					{
...
{
	$Date = Get-Date -format "dd-MMM-yyyy"
	$ActualDCWFileName = "Test" #To replace with real code
...
function QMWCheckNintexWorkflows ([string]$WAOrRC, [string]$QMWSiteURL)
{
#QMWCheckNintexWorkflows: Check to see what Nintex Workflows were deployed - Sharhad
	...
	$tmmSiteURL = $targetURL + "TMM"
	$cmmSiteURL = $targetURL + "CMM"
	$cdmSiteURL = $targetURL + "CDM"
	
	$qmwWeb = Get-SPWeb ($targetURL)
	
	$workflowsInfo =  $XMLConfig.XML.NintexWorkFlowArray.WorkflowInfo
	$workflowArray = @()
	foreach($item in $workflowsInfo)
	{
		if ($item.SubSiteURLCode -eq "CDM") {$targetWeb = (Get-SPWeb ($cdmSiteURL));} elseif ($item.SubSiteURLCode -eq "TMM") {$targetWeb = (Get-SPWeb ($tmmSiteURL));} elseif ($item.SubSiteURLCode -eq "CMM") {$targetWeb = (Get-SPWeb ($cmmSiteURL));}
		$SubSiteURLCode = $item.SubSiteURLCode		
		
		if ($item.WorkflowType -eq "List")
		{
		...
		elseif ($item.WorkflowType -eq "Site")
		{
			$workflowInList = $targetWeb.WorkflowAssociations.Name
			$workflowInList = $workflowInList | select -uniq 
			$workflowName = $item.WorkflowName
			foreach ($wfname in $workflowInList)
			{
	...
	foreach ($workflow in $workflowsInfo)
	{
		...
		$row["Value"] = "Does not Exists"
		$row["Value Required"] = "Exist"
		$row["Pass or Fail"] = "FAIL" 
...
function QMWCoSignFeatures ([string]$WAOrRC, [string]$QMWSiteURL)
{
	Write-Output "`n--- Checking CoSign Features ---"
	...
	$CoSignFeature = $XMLConfig.XML.CoSignFeatures.Feature
	if ($CoSignFeature.Count -gt 0){
		$features = (Get-SPFeature -Site $targetURL).GetTitle(1033)
		foreach ($feature in $features)
		{
			if ($feature -match "CoSign"){
				$row = $CoSignFeaturesTable.NewRow()
				$row["CoSign Feature"] = $feature
				$row["Expected Value"] = "Activated"
				$row["Actual Value"] = "Deactivated"
				$row["Pass or Fail"] = "FAIL"
				$row["Date"] = $Date
				foreach ($featureName in $CoSignFeature.FeatureName){
					if ($feature -match $featureName){
						$row["Actual Value"] = "Activated"
						$row["Pass or Fail"] = "PASS"
					}
				}
				$CoSignFeaturesTable.Rows.Add($row)	
			}
		}
	}
}

function QMWCheckCoSignSettings ([string]$WAOrRC, [string]$QMWSiteURL)
{
#QMWCheckCoSignSettings: Check CoSign library settings in QMW - Sharhad
...
{
#QMWCheckSecureStore: Check Secure Store in Central Admin Michael
...
function QMWContentTypeAssociation ([string]$WAOrRC, [string]$QMWSiteURL)
{
...
function QMWCheckSharePointSolutions
{
...
function QMWCheckHelpCenter([string]$WAOrRC, [string]$QMWSiteURL)
{
#QMWCheckHelpCenter: Check QMW Help Centers - Shovan
		...
		foreach ($settingToCheck in $helpCenterSettings)
		{
			...
			if ($settingToCheckSettingValue -eq "_ALL_")
			{
				foreach ($item in $targetList.items)
				{
					...
					#Write-Host "Status code was $statusCode"
					#add to table, if 200
...
#QMWVerification: Main QMW verification function that calls all the verification sub functions
	...
	#We are assuming that the $QMWRCURL ends with /, double check this and ensure it does
	#Check the last character of the site collection URL string
	$lastChar = $QMWRCURL.substring($QMWRCURL.length - 1, 1)
	...
	QMWDataConnection $WAOrRC $QMWURL #qmw
	QMWCheckWorkflowConstants $WAOrRC $QMWURL
	QMWCheckSharePointFeatures $WAOrRC $QMWURL
	QMWCheckNintexWorkflows $WAOrRC $QMWURL #all
	QMWCoSignFeatures $WAOrRC $QMWURL #qmw
	QMWCheckCoSignSettings $WAOrRC $QMWURL #tmm, cdm
	QMWContentTypeAssociation $WAOrRC $QMWURL
	QMWAttachmentSeperator $WAOrRC $QMWURL
	QMWCheckHelpCenter $WAOrRC $QMWURL #tmm, cdm
	
	#Call the functions that can apply to either the Work Area or RC for the RC
	$WAOrRC = "RC"
	QMWCheckSiteSettings $WAOrRC $QMWRCURL #qmw
	QMWCheckLibrarySettings $WAOrRC $QMWRCURL #qmw
	QMWCheckSharePointFeatures $WAOrRC $QMWRCURL
...
# GenerateReport: The main function (choice number 2) which Generates the report as a proof of what the script did
{
	...
	#CoSign Features Fragment Formatting
	$CoSignFragment = $CoSignFeaturesTable | ConvertTo-HTML "CoSign Feature","Expected Value","Actual Value","Pass or Fail","Date" -fragment
	$CoSignFeaturesHTML = "<br> CoSign Features:<br>$CoSignFragment<br><hr>"
	...
	#Attachment Seperator Fragment Formatting
	$AttachmentSeperatorFragment = $AttachmentSeperatorTable | ConvertTo-HTML "File Name","List Name","Expected Value","Actual Value","Pass or Fail","Date" -fragment
	$AttachmentSeperatorHTML = "<br>Attachment Seperator Association:<br>$AttachmentSeperatorFragment<br><hr>"
...
"@
	...
	$BodyTag = "$Bodystart $GeneralInfoHTML $PrerequisitesHTML $SharepointFeaturesHTML $DataConnectionsHTML $InfoPathFormServicesSettingsHTML $InfoPathFormTemplatesHTML $SiteSettingsHTML $LibrarySettingsHTML $SPSolutionsTableHTML $WorkflowConstantHTML $DeploymentHTML $DeploymentLogFilesFragmentHTML $RecordCenterSettingsHTML $WorkflowTableHTML $CoSignFeaturesHTML $CoSigSettingsHTML $ContentTypeHTML $AttachmentSeperatorHTML $HelpCenterHTML $TranscriptHTML"
...
#TODO: delete commented out tables, leaving as examples for right now
<#
